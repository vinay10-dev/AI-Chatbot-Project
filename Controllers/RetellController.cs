...
{
    // Dummy data
    private static readonly Dictionary<string, dynamic> Orders = new()
    {
        {
            "101", new {
                Id = "101",
    ...
    // 🔹 POST API (n8n compatible)
    [HttpPost("order-status")]
    public IActionResult GetOrderStatus([FromBody] OrderStatusRequest request)
    {
        ...
        if (!Orders.ContainsKey(request.order_id))
        {
            return NotFound(new
            {
                status = "not_found",
        ...
        var order = Orders[request.order_id];
